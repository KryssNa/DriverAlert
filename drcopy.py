...
class VideoFrameHandler:
    ...
    def process(self, frame: np.array, thresholds: dict):
        ...
        def update_tracker(self, condition, tracker_key, alarm_text):
            if condition:
                elapsed_time = time.perf_counter() - self.state_tracker["start_time"]
                self.state_tracker[tracker_key] += elapsed_time
                self.state_tracker["COLOR"] = self.RED
                if self.state_tracker[tracker_key] >= thresholds["WAIT_TIME"]:
                    self.state_tracker["play_alarm"] = True
                    plot_text(frame, alarm_text, ALM_txt_pos, self.state_tracker["COLOR"])
            else:
                self.state_tracker[tracker_key] = 0.0
            self.state_tracker["start_time"] = time.perf_counter()
        ...
        if results.multi_face_landmarks:
            ...
            if EAR < thresholds["EAR_THRESH"] or MAR > thresholds["MAR_THRESH"]:
                ...
                if self.state_tracker[time_key] >= thresholds["WAIT_TIME"]:
                    self.state_tracker["play_alarm"] = True
                    alarm_text = "WAKE UP! WAKE UP" if EAR < thresholds["EAR_THRESH"] else "Yawning Detected"
                    plot_text(frame, alarm_text, ALM_txt_pos, self.state_tracker["COLOR"])
            ...
            #         self.state_tracker["play_alarm"] = True
            #         plot_text(frame, "WAKE UP! WAKE UP", ALM_txt_pos, self.state_tracker["COLOR"])
            # if MAR > thresholds["MAR_THRESH"]:  # Check if MAR is greater than the threshold
            #     end_time = time.perf_counter()
            #     self.state_tracker["YAWNING_TIME"] += end_time - self.state_tracker["start_time"]
            #     self.state_tracker["start_time"] = end_time
            #     if self.state_tracker["YAWNING_TIME"] >= thresholds["WAIT_TIME"]:
            #         self.state_tracker["play_alarm"] = True
            #         plot_text(frame, "Yawning Detected", ALM_txt_pos, self.state_tracker["COLOR"])
            else:
                self.state_tracker["start_time"] = time.perf_counter()
                self.state_tracker["start_time"] = time.perf_counter()
                self.state_tracker["DROWSY_TIME"] = 0.0
            ...
            plot_text(frame, DROWSY_TIME_txt, (10,375), self.state_tracker["COLOR"])
